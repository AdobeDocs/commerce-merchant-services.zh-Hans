...
1. 下 *构建规则*，选择 **条件** ，并按照说明完成该声明。

   * 搜索查询包含 — 输入购物者查询中必须包含的文本字符串。 “匹配”设置确定购物者的查询与目录匹配的程度。 选项：<br /> 任意 — 购物者的查询文本的任何部分都可以匹配条件。<br />全部 — 购物者的所有查询都必须符合条件。
   * 搜索查询是 — 输入与购物者的查询完全匹配的文本字符串。 例如：“yoga pants”。 规则 `Search query is` 和匹配 `All` 只能有一个条件。
   * 搜索查询开头为 — 输入必须在购物者查询开头的字符或文本字符串。
   * 搜索查询结尾为 — 输入必须在购物者查询结尾的字符或文本字符串。

   结果会立即显示在 *测试规则* 窗格中按优先级进行编号。 您可以使用 *每行结果数* 用于更改每行中产品数量的右上角的滑块。

   ![规则 — 简单](assets/rule-simple-test.png)

1. 要测试其他查询，请更改 *测试规则* 搜索框并按 **返回**.
最初，测试窗格从“条件”搜索框渲染查询。 但现在，它从测试查询框呈现查询。 测试窗格一次仅呈现一个查询。
...
排名结合了用户行为和网站统计数据，以确定产品排名。
商店所有者可以设置以下类型的排名策略：

![规则 — 匹配](assets/rules-ranking-type.png)

* 购买次数最多：此维度按过去7天内每个SKU的总购买量对产品进行排名。
* 添加到购物车次数最多 — 按照过去7天内的全部“添加到购物车”活动排名。
* 查看次数最多：按SKU对过去7天我的总查看次数进行排名。
* 为您推荐 — 使用 `viewed-viewed` 数据点 — 查看此SKU的购物者也查看了这些其他SKU
* 趋势：对于后台事件，回顾过去72小时内的页面查看事件；对于前台事件，回顾过去24小时的页面查看事件
* 无：产品按相关性排序
...
事件是在满足定义的条件时修改搜索结果的操作。 单个规则最多可以包含25个事件。
...
1. 在“测试”窗格中单击并拖动产品。 将其拖放到所需位置。 “事件”窗格中会自动填充“产品”和“位置”字段。
...
1. 下 *事件*，选择 **事件** 在满足相关条件时发生。

   例如，选择 `Hide a product`. 然后，输入要隐藏的产品名称。 建议在键入产品时使用该功能。

1. 对于多个事件，选择满足条件时要触发的任何其他事件。

## 其他详细信息

此处输入的信息将显示在 [规则详细信息](rules-workspace.md) 面板。
...
1. 尽管活动规则会立即生效，但可能需要等待最多15分钟才能刷新店面中的缓存查询结果。
...
| 条件 | 描述 |
|--- |--- |
| 搜索查询包含 | 购物者的查询中包含的字符或文本字符串。 购物者的查询只需匹配单个字符即可满足此条件。 |
| 搜索查询为 | 与购物者的查询完全匹配的字符或文本字符串。 使用此条件时，无法组合具有多个条件的复杂查询。 |
| 搜索查询始于 | 购物者的查询以这个字符或文本字符串开头。 |
...
| 运算符 | 描述 |
|--- |--- |
| 或 | （默认）逻辑运算符 `OR` 比较两个条件并满足在至少有一个条件为true时触发事件的要求。 |
| 和 | 逻辑运算符 `AND` 比较两个条件并满足要求，在两个条件都为真时触发事件。 |

### 匹配运算符

| 运算符 | 描述 |
|--- |--- |
| 任意 | 将规则中的所有逻辑运算符更改为 `OR` 和会返回匹配产品集。 |
| 全部 | 将规则中的所有逻辑运算符更改为 `AND` 和会返回匹配产品集。 |
...
| Bury | 将搜索结果中的SKU或SKU范围下移。 每个页面在测试搜索结果中均标有“掩埋”预览标记。 |
