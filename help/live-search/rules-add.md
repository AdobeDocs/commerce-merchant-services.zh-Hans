...
排名将用户行为和网站统计信息相结合，来确定产品排名。
商店所有者可以设置以下类型的排名策略：

![规则 — 匹配](assets/rules-ranking-type.png)

* 购买次数最多：这会按过去7天的每个SKU总购买量对产品进行排名。
* 添加到购物车的次数最多 — 按过去7天内“添加到购物车”活动总数的顺序排名。
* 查看次数最多：对过去7天中每个SKU的总查看次数进行排名。
* 为您推荐 — 使用 `viewed-viewed` 数据点 — 查看了此SKU的购物者也查看了这些其他SKU
* 趋势：回顾过去72小时内的页面查看事件（后台事件）和24小时的前台事件
* 无：产品按相关性订购

1. 为规则选择策略类型。 “测试您的规则”窗口会显示预期结果。

## 添加事件

事件是在满足定义的条件时修改搜索结果的操作。 一个规则最多可包含25个事件。
...
1. 在“测试”窗格中单击并拖动产品。 将其拖放到所需位置。 “产品”和“位置”字段会自动填充到“事件”窗格中。

   ![规则 — 匹配](assets/rule-event-pin-product.png)

您还可以单击固定图标以将产品固定到其当前位置。 使用省略号上下文菜单“固定到顶部”或“固定到底部”。
...
1. 在 *事件*，选择 **事件** 在满足相关条件时进行。
   ...
   例如，选择 `Hide a product`. 然后，输入要隐藏的产品名称。 在您键入时，系统会推荐产品。
