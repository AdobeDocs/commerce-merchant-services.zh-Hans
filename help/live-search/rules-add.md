...
一个规则最多可以具有10个条件。 连接两个条件的逻辑运算符基于当前 *匹配* 设置。 默认情况下， *匹配* is `All` 逻辑运算符是 `AND`.
...
在本例中，除了搜索“瑜伽裤”之外，还有两个单独的查询可搜索“yoga”或“pants”。 此规则不太具体，因此在店面中触发的频率要高于其他规则。
...
事件是在满足条件时更改搜索结果的操作。 一个规则最多可包含二十五个事件。

1. 在 *事件*，选择 **事件** 在满足关联条件时进行。
...
1. 在 *详细信息*，输入 **名称** 规则。
1. 输入摘要 **描述** 规则。
1. 输入 **开始日期** 和 **结束日期** 规则何时激活或从日历中选择日期。
...
| Event | 描述 |
|--- |--- |
| 提升 | 在搜索结果中将SKU或SKU范围移得更高。 在测试搜索结果中，每个组件都带有“已提升”的预览徽章。 |
| 伯里 | 在搜索结果中将SKU或SKU范围移低。 在测试搜索结果中，每个测试都带有“隐藏”预览徽章。 |
