...
此特定用例侧重于使用来自[!DNL Commerce]实例的数据创建放弃的购物车电子邮件，并将其发送给Adobe[!DNL Journey Optimizer]。
...
>[!IMPORTANT]
>
>出于演示目的，请使用您的[!DNL Commerce]沙盒环境，这样您就不会用发送给Experience Platform的店面和后台事件数据稀释生产事件数据。
