...
下面列出了安装时可用的Commerce行为事件 [!DNL Data Connection] 扩展。 这些事件收集的数据将发送到Adobe Experience Platform。 您还可以创建 [自定义事件](custom-events.md) 收集未开箱即用的其他数据。
...
>[!NOTE]
>
>所有行为事件包括 [`identityMap`](https://experienceleague.adobe.com/docs/experience-platform/xdm/field-groups/profile/identitymap.html) 字段，其中包括购物者的电子邮件地址（如果可用）和ECID。

## 店面活动

店面活动从购物者在网站上的互动中捕获数据，并包括以下活动 [`addToCart`](#addtocart)， [`pageView`](#pageview)， [`createAccount`](#createaccount)， [`editAccount`](#editaccount)， [`startCheckout`](#startcheckout)， [`completeCheckout`](#completecheckout)， [`signIn`](#signin)， [`signOut`](#signout)，等等。 店面事件仅适用于简单且可配置的产品。
...
搜索事件会提供与购物者意图相关的数据。 洞察购物者的意图有助于商家了解购物者如何搜索商品、点击内容，最终购买或放弃。 例如，如果您希望定位现有购物者，这些购物者搜索您的热门产品，但从未购买该产品，您可能会如何使用此数据。 您必须安装 [[!DNL Live Search]](../live-search/install.md) 用于访问这些事件的扩展。
...
![适用于Adobe Commerce的B2B](../assets/b2b.svg) 对于B2B商家，您必须 [安装](install.md#install-the-b2b-extension) 该 `experience-platform-connector-b2b` 用于访问这些事件的扩展。
