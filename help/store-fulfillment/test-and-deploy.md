...
- 已完成 [常规配置](enable-general.md) 用于“商店履行”服务。

- [添加并验证沙盒和生产环境的帐户凭据和连接详细信息](connect-set-up-service.md#configure-store-fulfillment-account-credentials)

- 确认 [Adobe Commerce集成](connect-set-up-service.md#configure-store-fulfillment-account-credentials) ，可获得授权。
...
1. 在商店网格中，验证是否已将商店标记为 `Synced` 创建测试订单之前。
...
| 前端结帐</br>签到表单 | 签到体验 | 客户、管理员 |
| 商店协助应用程序 | 订购</br>选取</br>Stage</br>和移交 | 存储关联 |
...
<td>将更新保存到现有的取车库来源。</td>
<td>成功操作后， <code>Is Synced</code> “管理源”页更新中的列 <code>No</code> 到 <code>Yes</code>.</td>
</tr>
<tr>
...
<td>客户提交商店取货订单。</td>
<td><ul><li>在“管理订单”视图中， <strong>Adobe Commerce管理员用户</strong> 看到订单同步状态已更新为 <code>Sent</code></li><li>订单详细信息日志包含消息 <code>Order was sent to BOPIS solution for sync, it's not yet acknowledged yet.</code></li></ul></td>
</tr>
<tr>
...
<td>
<ul>
<li>订单历史记录已更新为 <code>We refunded $X online. Transaction ID: transactionID</code> 和 <code>Received Cancel acknowledgment from the BOPIS solution.</code></li>
<li>订单状态为 <code>Closed</code>. （我们已经设置了“付款审核”。）</li>
<li>在Adobe Commerce中创建的贷项通知单。 （请等待cron正常工作。）</li>
<li>如果所有物品都已捡料，则准备好接收电子邮件 <code>DISPLAY COMMENT HISTORY</code> 显示 <code>Order is ready for pickup</code> (<code>CUSTOMER NOTIFIED</code> 标志为 <code>true</code>.)</li>
...
<li>如果某些物料未领料，请确认 [!UICONTROL Ready for Pickup] 将显示包含“无”选择或退款部分的电子邮件。 <code>DISPLAY COMMENT HISTORY</code> 显示 <code>Order is ready for pickup, but some items not available.</code>.</li>
<li><code>CUSTOMER NOTIFIED</code> 标志为 <code>true</code>.</li>
</ul>
</td>
</tr>
...
<li>等待订单同步。</li>
<li>检查是否已创建发票（如果授权并捕获）以及是否已收到发票电子邮件。</li>
<li>转到Postman并运行准备收取请求，将所有产品设置为 <code>picked</code> 替换为 <code>0 qty</code>.</li>
</ol>
</td>
<td>
<ul>
...
<tr>
<td><strong>分配（分配期间） </br></br>完全取消（所有产品都设置为已拒绝）</strong>
</td>
<td>
<ol>
<li>下订单。</li>
...
<tr>
<td><strong>分配（分配期间）</br></br>部分取消</br>（有些产品已经配发，有些则被拒绝。）</strong>
</td>
<td>
<ol>
<li>下订单。</li>
...
<li>打开您的邮箱。 查找“准备取货”电子邮件，然后选择 <code>Confirm Arrival</code>.</li>
<li>签到。</li>
...
<td>
<ul>
<li>RMA已创建并显示在以下 <strong>[!UICONTROL Returns]</strong> 选项卡。</li>
<li>客户收到了RMA确认电子邮件。</li>
...
<ul>
<li>已成功创建帐户。</li>
<li>新的用户帐户将显示在 [!UICONTROL Store Fulfillment Users] 仪表板。</li>
<li><strong>存储关联</strong> 使用新的用户帐户登录到“商店助手”应用程序。</li>
</ul>
</td>
</tr>
<tr>
...
<li>验证当至少有一个子产品具有下列条件时，是否已为客户启用交付方法 <code>qty</code> 设置为 <code>0.</code></li>
...
<td>
<ul>
<li>验证是否至少有一个子产品具有 [!UICONTROL Available for Store Pickup] 已禁用，客户无法使用“商店见面交货”选项。</li>
...
- 备用提货联系人 — 验证用于添加 [!UICONTROL Alternate Pickup Contact] 并选择 [!UICONTROL Preferred Contact] 在商店提货单上。

- 签入表单 — 验证用于提交商店提货单的签入请求的工作流。

**功能领域：** 购物车结帐、商店提货单的登记表</br>
**角色：** 管理员、客户、商店关联</br>
**测试类型：** 全部为正
...
<td>
客户提交具有“店内提货”选项的订单。 在结账过程中，客户添加 [!UICONTROL Alternate Pickup Contact].</td>
<td>在结账过程中，客户会看到 [!UICONTROL Preferred Contact] 配送步骤上的选项。</td>
</td>
</tr>
<tr>
<td><strong>备用取车联系人详细信息，签到</strong>
</td>
<td>
客户提交具有“店内提货”选项的订单。 在结账过程中，客户选择 [!UICONTROL Alternate Pickup Contact] 在运输步骤中。
</td>
<td>客户看到输入选项以输入联系人详细信息： [!UICONTROL First name]， [!UICONTROL Last name]， [!UICONTROL Phone]、和 [!UICONTROL Email].</td>
</tr>
<tr>
<td><strong>备用接送、登记电子邮件</strong>
</td>
<td>客户提交具有“店内提货”选项的订单。 在结账过程中，客户选择 [!UICONTROL Alternate Pickup Contact] 在配送步骤中，添加联系人详细信息并提交订单。</td>
...
<tr>
<td><strong>替代提货联系人、商店关联订单视图</strong>
</td>
<td>客户提交具有“店内提货”选项的订单。 在结账过程中，客户选择 [!UICONTROL Alternate Pickup Contact] 在配送步骤中，添加联系人详细信息并提交订单。</td>
...
<td>店内提货单已标记为可在FaaS中提货。 客户收到“Ready for Pickup（准备好取货）”电子邮件，并选择 [!UICONTROL Confirm Arrival].</td>
<td>客户看到订单的“签到”表格。</td>
</tr>
</tbody>
</table>
...
<tr>
<td>
<strong>单订单领料 — 快乐路径，路边领料</strong></td>
