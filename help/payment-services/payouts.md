...
* **[支付数据可视化视图](#payouts-data-visualization-view)** — 支付服务主页上提供的图表，它直观地显示了“支付”报表视图中每日合计金额的表示形式
* **[付款报表视图](#payouts-report-view)** — 显示所有交易的详细付款信息的付款中提供的报告

“付款”视图显示全面的付款信息，使您对付款金额、已处理数量和财务对账事务处理级别的详细报告完全透明。
...
支付服务主页中提供了支付数据可视化视图。 它是详细表格中每日汇总金额的直观表示形式 [付款报表视图](#payouts-report-view).
...
从“付款数据”可视化视图中，您可以通过选择日期范围来自定义要查看的付款交易记录的时间范围：

1. 在 _管理员_ 侧栏，转到 **[!UICONTROL Sales]** > **[!UICONTROL Payment Services]**. “支付”数据可视化视图显示在“支付”部分。
...
选定日期范围的交易金额显示在“付款数据”可视化视图的左侧。 选定日期范围的日期将显示在视图底部。 如果某个特定日期没有付款，该日期将不会出现。

支付数据可视化视图包括以下信息。

| 数据 | 描述 |
| ------------ | -------------------- |
| [!UICONTROL Transaction amount] | 指定时间范围内的交易金额范围；Y轴上的数据（左） |
| 日期范围 | 指定时间范围的日期范围；X轴上的数据（底部） |
| 贷项 | 指定时间范围的付款 |
| 借记 | 指定期限借记（退款） |
| 移动平均值 | 指定时间范围内每个日期的平均支出表示 |
| 范围净值 | 指定时间范围（范围）的净支付额 |

## 付款报表视图

“支付”报表视图可在“支付服务”的“支付”视图中使用。 它包括所有关于你的商店支付费用的可用信息。 的 [支付数据可视化视图](#payouts-data-visualization-view) 在Payment Services Home中，在这个更详细的报表视图中直观地显示了每日汇总金额。

在 _管理员_ 侧栏，转到 **[!UICONTROL Sales]** > **[!UICONTROL Payment Services]** > **[!UICONTROL Payouts]** 查看详细的表格支付报告视图。

![管理员中的支付交易](assets/payouts-report-new.png)

您可以根据本主题中的部分配置此视图，以最好地显示您希望查看的数据。
...
在“付款”报表视图中，您可以通过输入特定日期或从日期选取器中选择日期范围，自定义要查看的付款交易记录的时间范围：
...
默认情况下，“付款”报表视图显示大多数可用的信息列。 但是，您可以自定义在报表中看到的列。
...
1. 要自定义在报表中看到的列，请勾选或取消选中列表中的列。
   ...
   “付款”报表视图将立即显示您在“列”设置菜单中所做的任何更改。 列首选项将会保存，并且在您离开报表视图时保持有效。
...
| [!UICONTROL Commerce order] | 商务订单ID <br> <br>查看相关 [订购信息](https://docs.magento.com/user-guide/sales/orders.html)，请单击ID。 |
| [!UICONTROL Commerce trans] | 商务交易ID |
